...
"""Define the equation that calibrates chlorophyll.

Chlorophyll is calculated using the Volt 2 output from each SASS CTD. Calibration coefficients
consist of the clean water offset (CWO) and scale factor (SF), which are determined during
calibration of the instrument by Seabird (SBE).

Upon receipt of a newly  calibrated instrument from seabird, the CWO and SF sheets are
stored as a hardcopy in a binder titled SASS FLUOROMETERS and as a PDF in the SASS shared
Google Drive.  When a new fluorometer is deployed, the CWO and SF for that fluorometer is
entered into the SASS Inventory and Cleaning google spreadsheet, along with the
...
def calibrate_chlorophyll(output, scale_factor=None, clean_water_offset=None, **kwargs):
    ...
    where:
    Clean Water Offset (CWO): signal output of the fluorometer in pure filtered de-ionized water (V)
    ...
    """
    chlorophyll = (output - clean_water_offset) * scale_factor
